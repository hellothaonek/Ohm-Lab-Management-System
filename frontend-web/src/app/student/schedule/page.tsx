...
export default function StudentSchedule() {
  ...
  useEffect(() => {
    ...
    const options = weekStarts.map((weekStart) => {
      const weekEnd = addDays(weekStart, 6)
      return { value: format(weekStart, "yyyy-MM-dd"), label: `${format(weekStart, "dd/MM")} - ${format(weekEnd, "dd/MM/yyyy")}` }
    })
    options.sort((a, b) => new Date(a.value).getTime() - new Date(b.value).getTime())
    setWeeks(options)

    const dates = Array.from({ length: 7 }).map((_, i) => format(addDays(selectedWeekStart, i), "yyyy-MM-dd"))
    setWeekDates(dates)
  }, [selectedWeekStart])

  const handleWeekChange = (value: string) => setSelectedWeekStart(new Date(value))
        ...
        <div className="grid grid-cols-8 gap-px border rounded overflow-hidden text-center text-sm">
          ...
          {days.map((d, index) => (
            <div key={d} className="bg-gray-100 font-bold p-2">
              <div>{d}</div>
              <div className="text-xs text-muted-foreground">{weekDates[index] ? format(new Date(weekDates[index]), "dd/MM") : ""}</div>
            </div>
          ))}
          ...
          {slots.map((time, slotIdx) => (
            <div key={`slot-${slotIdx}`} className="contents">
              ...
              {days.map((d) => (
                <div key={`${d}-${slotIdx}`} className="min-h_[80px]">
